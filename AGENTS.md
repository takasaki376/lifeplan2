...
このドキュメントは、Codex（自律エージェント/CLI）や協力開発者が `lifeplan2` の実装を安全に前進させるための作業規約です。
人間の判断が必要なところは「決定済みの方針」に従い、迷いが出た場合は “最小で進む” を優先してください。

---

## 0. TL;DR（最短の進め方）

1. チケット（例: B4, D5, G1-1）を読み、受け入れ条件（AC）を箇条書きにする
2. 影響範囲（app route / lib / repo / db schema）を特定する
3. 実装 → `yarn lint` / `yarn test`（存在する場合）→ `yarn build` を通す
...
- 単発試算ではなく、月次（収入・支出・資産/負債）を継続入力しながら、
  ライフイベント・住宅 LCC・改定履歴（バージョン）を“見直し続ける”アプリ。
- MVP の永続化は **IndexedDB**、将来 **Supabase** 移行を想定し、DB アクセスは Repository 層で抽象化する。
...
- Repo は「包みすぎない」。ただし文脈（planId/ym 等）を付与した `RepoError` を投げてよい。

### 4.2 トランザクション境界（withTx）

- **原則: usecase 側が tx を開始し、複数 Repo に同一 tx を渡す。**
...
- 画面間遷移（例: D5→G1）は `scenario` を引き継ぐ。
...
- public API（repo/usecase/calc）は入力/出力型を明確に。

### 6.2 UI（Next.js + shadcn/ui）

- IndexedDB アクセスを行うコンポーネントは `"use client"`。
- “空状態/エラー/ローディング” を必ず分岐する（Skeleton/Alert）。
...
- [ ] `yarn lint` が通る
- [ ] `yarn build` が通る
- [ ] 画面の手動テスト手順を PR に記載（URL/操作/期待）
- [ ] 破壊的変更（schema 変更等）はマイグレーション方針を README か doc に追記
...
- ✅ 既存の Vitest/ESLint/Next に沿うもの
- ❌ 状態管理ライブラリの追加（Redux 等）は MVP では不要
- ❌ 重い日付ライブラリを無条件追加（必要なら最小で）
...
- 例: schema 変更が必要なら、後方互換（version bump）を優先する
