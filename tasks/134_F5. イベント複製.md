...
- 変更する項目：
  ...
  - createdAt：新規
  - updatedAt：新規
  - title：`{元タイトル}（コピー）` を付与（任意だが推奨）
- cadence=once の場合は durationMonths をクリアしても OK（ただし元が once なら通常 undefined の想定）
...
- [ ] 複製後に新イベントの編集画面へ遷移できる（または一覧へ戻る方針でも可）
- [ ] 新イベントは id/createdAt/updatedAt が新規である
- [ ] タイトルに（コピー）が付与される（採用した場合）
