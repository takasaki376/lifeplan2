...
- 毎月イベントで期間を指定する場合、期間が 1 以上であることを保証する
...
- cadence = `monthly` の場合：
  - 「期間なし（ずっと）」→ durationMonths は undefined（OK）
  - 「期間を指定」→ `durationMonths >= 1` 必須
- cadence = `once` の場合：
  - durationMonths は未使用（入力されていても無視/削除）

### 追加の軽量チェック（任意・推奨）

- startYm の形式が `YYYY-MM` であること
- （終了月入力がある UI の場合）終了月 >= 開始月
...
- [ ] 毎月イベントで期間指定時、期間が 1 未満なら保存できない（エラー表示される）
- [ ] cadence が単発のとき、期間入力が保存に影響しない（無視 or クリアされる）
